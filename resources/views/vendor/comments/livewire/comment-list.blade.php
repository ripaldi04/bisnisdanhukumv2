...
<div x-data="{
...
}" @unauthorized-comment-updated.window="$wire.$refresh" class="space-y-6">
    ...
    <div class="flex flex-col gap-y-2 sm:flex-row sm:items-center sm:justify-between">
        @if (($total > 1 || $filter === 'own') && config('comments.show_filters'))
            <div class="flex gap-x-2 sm:gap-x-3 overflow-auto">
                <div class="w-14"></div>
                <x-comments::chip wire:click="setSortBy('{{ Sort::TOP->value }}')"
                    wire:loading.class="!pointer-events-none" :active="$sortBy === Sort::TOP">
                    {{ __('Top') }}
                </x-comments::chip>
                <x-comments::chip wire:click="setSortBy('{{ Sort::LATEST->value }}')"
                    wire:loading.class="!pointer-events-none" :active="$sortBy === Sort::LATEST">
                    {{ __('Newest') }}
                </x-comments::chip>
                <x-comments::chip wire:click="setSortBy('{{ Sort::OLDEST->value }}')"
                    wire:loading.class="!pointer-events-none" :active="$sortBy === Sort::OLDEST">
                    {{ __('Oldest') }}
                </x-comments::chip>
                <x-comments::chip wire:click="setSortBy('{{ Sort::REPLIES->value }}')"
                    wire:loading.class="!pointer-events-none" :active="$sortBy === Sort::REPLIES">
                    {{ __('Replies') }}
                </x-comments::chip>

                <x-comments::chip wire:click="setFilter('own')" wire:loading.class="!pointer-events-none"
                    :active="$filter === 'own'">
                    {{ __('My Comments') }}
                </x-comments::chip>
            </div>
    ...
    <div wire:loading.flex class="flex items-center gap-x-2 sm:gap-x-4">
        ...
        <x-comments::spin class="!size-5 !text-yellow-500" />
    </div>

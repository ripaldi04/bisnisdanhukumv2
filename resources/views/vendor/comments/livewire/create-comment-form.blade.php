        ...
        @if ($model->guestModeEnabled() && !$this->secureGuestMode->enabled())
            <div class="flex flex-col gap-x-8 sm:flex-row">
                <div class="flex w-full flex-col">
                    <x-comments::input wire:model="name" :shouldDisable="$limitExceeded" placeholder="{{ __('Comment as') }}" />
                    <div class="min-h-6">
                        @if ($errors->has('name'))
                ...
                @if (config('comments.guest_mode.email_enabled'))
                    <div class="flex w-full flex-col">
                        <x-comments::input wire:model="email" :shouldDisable="$limitExceeded" type="email"
        ...
        @if (!$limitExceeded)
            @if (!$this->guestModeEnabled && $loginRequired)
                <div>
                    <span>
                        {{ __('Please') }}
                        <x-comments::link wire:click.prevent="redirectToLogin(window.location.href)"
            ...
            @elseif(!$this->secureGuestMode->allowed())
                <div x-data="{ showEmailField: false }" id="verify-email-button">
                    <span x-show="!showEmailField" x-transition>
                        {{ __('Please') }}
                        <x-comments::link @click="showEmailField=true" class="font-bold text-yellow-600" type="button">
                    ...
                    <div x-show="showEmailField" x-transition class="flex flex-col gap-y-2">
                        <div class="flex flex-col gap-x-8 sm:flex-row">
                            <div class="flex w-full flex-col">
                                <x-comments::input wire:model="name" placeholder="{{ __('Comment as') }}" />
                                <div class="min-h-6">
                                    @if ($errors->has('name'))
                                        <span class="align-top text-xs text-red-500 sm:text-sm">
                                            {{ __($errors->first('name')) }}
                            ...
                            <div class="flex w-full flex-col">
                                <x-comments::input wire:model="email" type="email"
                                    placeholder="{{ __('Email') }}" />
                                <div class="min-h-6">
                                    @if ($errors->has('email'))
                                        <span class="align-top text-xs text-red-500 sm:text-sm">

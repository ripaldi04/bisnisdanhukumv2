<div>
    <form wire:submit.prevent="create" class="w-full" method="POST">
        ...
        <div class="min-h-6">
            ...
            <div x-cloak x-data="message(@js($editorId))"
        ...
        @if (!$limitExceeded)
            ...
            @elseif(!$this->secureGuestMode->allowed())
                <x-comments::link type="a"
                    route="#verify-email-button">{{ __('Please verify your email to reply') }}</x-comments::link>
            @else
                <div class="flex gap-x-2">
                    <x-comments::button loadingTarget="create" class="w-full sm:w-auto" size="sm">
                        {{ __('Create') }}
                    </x-comments::button>
                    <x-comments::button wire:click="discard" loadingTarget="discard" type="button"
                        class="w-full sm:w-auto" size="sm">
                        {{ __('Discard') }}
                    </x-comments::button>
                </div>
            @endif

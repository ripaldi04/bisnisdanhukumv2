<div>
    @if ($show)
        <div class="scrollbar max-h-96 min-h-96 w-full overflow-auto rounded bg-gray-200 shadow-lg">
            ...
            }" @keydown.up="$focus.wrap().previous();" @keydown.down="$focus.wrap().next()"
                @keydown.up.window="setFocusFirst()" @keydown.down.window="setFocusLast()" class="space-y-1">
                @foreach ($users as $user)
                    <button wire:click="userSelected(@js($user->name))"
            ...
            <div wire:loading.flex class="mt-4 flex justify-center">
                <x-comments::spin class="size-6 text-center !text-yellow-500" />
            </div>
        </div>
    @endif
</div>

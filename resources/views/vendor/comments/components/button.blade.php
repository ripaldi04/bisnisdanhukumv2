...
@if ($type === 'submit')
    <button wire:loading.remove wire:dirty.remove.attr="disabled" wire:dirty.class="cursor-pointer"
        @if ($dirtyTarget || $loadingTarget) wire:target="{{ $dirtyTarget }},{{ $loadingTarget }}" @endif disabled
        type="submit"
    ...
    <button wire:loading.inline-flex @if ($loadingTarget) wire:target="{{ $loadingTarget }}" @endif
...
@if ($type === 'button')
    <button wire:loading.remove @if ($loadingTarget) wire:target="{{ $loadingTarget }}" @endif
        type="button"
        {{ $attributes->merge(['class' => $class . $severity . 'inline-flex justify-center items-center gap-x-2 font-semibold rounded border border-transparent text-white disabled:opacity-50 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600']) }}>
        {{ $slot }}
    </button>

    <div wire:loading.delay.longer @if ($loadingTarget) wire:target="{{ $loadingTarget }}" @endif>
        <button wire:loading.inline-flex @if ($loadingTarget) wire:target="{{ $loadingTarget }}" @endif

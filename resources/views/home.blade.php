                ...
                <div class="flex flex-col gap-4">
                    ...
                    <div class="text-center mx-auto">
                        <p class="text-xl font-bold mb-2 max-w-sm"><a
                                href="{{ route('articles.show', $article->slug) }}">{{ $article->title }}</a></p>
                        <p class="text-center text-slate-500">{{ $article->updated_at->format('d M Y') }}</p>
                    </div>
                </div>
            @endforeach
        </div>
                ...
                <div class="flex flex-wrap justify-center gap-12">
                    @foreach ($benefits as $benefit)
                        <div class="flex flex-col items-center space-y-3 max-w-sm">
                            <!-- Ikon -->
                            <img src="{{ Storage::url($benefit->icon) }}" alt="" class="icon-blue-to-gold">
                            <!-- Judul -->
                            <h3 class="font-bold text-xl">{{ $benefit->title }}</h3>
                            <!-- Deskripsi -->
            ...
            <div class="grid gap-8 md:grid-cols-3">
                @foreach ($uniquenesses as $uniqueness)
                    <div class="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
                        ...
                        <h3 class="text-xl font-semibold mb-4 text-gray-800">{{ $uniqueness->title }}</h3>
                        <p class="text-gray-600">{{ $uniqueness->description }}</p>
                    </div>
                @endforeach
            </div>
        </div>
